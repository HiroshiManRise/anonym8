...
# STOP ####
function stop_mac {
	echo -e "\n$GREEN*$BLUE Stop Mac Address Spoofing\n"
	sudo service network-manager stop
	sleep 1
	echo -e "$GREEN*$BLUE wlan0 MAC address:\n"$GREEN	
	sleep 1
...
## IP ####
function status_ip {
	echo -e "\n$GREEN*$BLUE Curent IP:\n"$GREEN
	curl icanhazip.com
	echo -e "$BLUE---------------\n"$RESETCOLOR
	sleep 1
	echo -e "$GREEN*$BLUE Related Infos:\n"$GREEN
	sleep 1
	curl ipinfo.io/
	sleep 1
}
...
function start {
	...
	if ! [ -f /etc/network/iptables.rules ]; then
		...
		echo -e "$GREEN*$BLUE iptables.rules saved...$RESETCOLOR\n"
		sleep 2
	fi
	...
	sleep 2
	echo -e 'nameserver 127.0.0.1\nnameserver 213.73.91.35\nnameserver 87.118.100.175' > /etc/resolv.conf
	echo -e "$GREEN*$BLUE resolv.conf: Chaos Computer Club & German Privacy Foundation$GREEN [ACTIVATED]\n"$RESETCOLOR        
	sleep 2
...
function stop {
	...
	echo -e "$GREEN*$BLUE Services Successfully Reloaded\n"
	sleep 3
...
function change {
	...
	sleep 1
	echo -e "$GREEN*$BLUE Curent IP:"$GREEN
	curl icanhazip.com
